...
_GPU_SPECS_DATABASE: dict[str, dict[str, object]] = {
    ...
    # SM121 — uses Ampere-era mma.sync (NOT datacenter Blackwell tcgen05/WGMMA)
    # 128 GB unified LPDDR5x shared between CPU and GPU via NVLink-C2C
    # No FlashAttention, no TMEM, no GPUDirect RDMA
    "NVIDIA GB10": {
        "name": "NVIDIA GB10",
        "architecture": "Blackwell",
        "peak_fp32_tflops": 14.0,  # 48 SMs × 128 FP32 cores/SM × 2 ops × 1.1 GHz
        "peak_fp16_tflops": 209.0,  # 192 tensor cores, 5th gen, FP16 without sparsity
        "peak_bf16_tflops": 209.0,  # Same as FP16 via tensor cores
        "peak_memory_bw_gbps": 273,  # LPDDR5x unified memory bandwidth
        "sm_count": 48,
        "max_threads_per_sm": 2048,
        "l1_cache_kb": 128,  # 128 KB shared memory/SM (NOT 228 KB like datacenter)
        "l2_cache_mb": 8,  # Estimated — not officially published
        "memory_gb": 128,  # Unified LPDDR5x (119.68 GB CUDA-visible)
        "memory_type": "LPDDR5x",
